...
interface sTaskQualificationOptions {
  allowPlanning?: boolean;
  worker?: iEngineStep;
}

/** Routes one task either into Planner or directly into Worker. */
export class TaskQualification extends EngineStep {
    ...
    qualify: this.point({
      ...
      response: async (result, dsl) => {
        ...
        if (qualification.type === 'multi' && this.allowPlanning) {
          const nestedQualification = new TaskQualification({
            allowPlanning: false,
            worker: this.worker,
          });
  ...
  public constructor(options: sTaskQualificationOptions = {}) {
    super();
