// PlanExecutor.ts
import type { ChangeExecution } from '@execution/ChangeExecution';
import type { ChangeWorkItem } from '@execution/State/ChangeState';
...
import type { PlanUpdater } from '@planner/PlanUpdater';
import type { RecoveryController, RecoveryDecision } from '@planner/RecoveryController';
import type { PlanStep, StepRequirementContract, TaskPlan } from '@planner/TaskPlan';
...
import { ChangeDefinitionCompiler } from '@planner/ChangeDefinitionCompiler';
import { FinalResultCompiler } from '@planner/FinalResultCompiler';
import { RequirementConstraintValidator } from '@planner/RequirementConstraintValidator';
import type { RequirementResolutionPlanner } from '@planner/RequirementResolutionPlanner';
import { parseWorkflowDataRef } from '@planner/WorkflowData';
import { PlannerContext } from '@planner/PlannerContext';
import { ContextComposer } from '@planner/ContextComposer';
...
export interface PlanExecutionState {
  ...
  executionContext: PlannerContext;
...
export class PlanExecutor {
  ...
  public constructor(
    ...
    private readonly changeExecution: ChangeExecution,
  ...
  public async run(state: PlanExecutionState): Promise<PlanRunResult> {
    ...
    while (state.execution.status === 'running' && state.planIndex < state.plan.steps.length) {
      ...
      if (step.type === 'edit-file') {
        if (!step.targetPath) {
          const recovered = await this.recover(state, 'edit-file has no targetPath');
          if (!recovered) return state.pauseReason ? 'paused' : 'finished';
          continue;
        }
        ...
        const work: ChangeWorkItem = {
          ...
          requirements: step.requirements?.map(({ ref, description, constraints }) => ({ ref, description, constraints })),
          changeDefinition,
          maxAttempts: step.maxAttempts,
        };

        try {
          const changeRun = await this.changeExecution.execute({
            work,
          ...
          state.stepAttempts = changeRun.state.attempt;

          if (changeRun.status === 'failed') {
            await this.logger.error('change-execution-error', { stepId: step.id, error: changeRun.error }, context);
            state.execution.addEvent('change-execution-error', { stepId: step.id, error: changeRun.error, history: changeRun.state.history });
            const recovered = await this.recover(state, `change-execution-error:${changeRun.error}`);
            if (!recovered) return state.pauseReason ? 'paused' : 'finished';
            continue;
          }

          const changes = changeRun.state.proposal ?? [];
          this.reporter.changes(changes.map((change) => change.path));
          const synthetic: StepResult = {
            goalSatisfied: true,
            findings: [`Validated and applied changes to: ${changes.map((change) => change.path).join(', ')}`],
            evidence: changes.map((change) => ({ path: change.path, fact: 'File change passed execution State/Option/Worker pipeline and was committed.' })),
            missing: [],
            facts: step.outputs.map((key) => ({
              key,
              value: `Validated and applied requested edit to ${changes.map((change) => change.path).join(', ')}`,
              evidence: changes.map((change) => ({ path: change.path, fact: 'File change passed execution State/Option/Worker pipeline and was committed.' })),
            })),
          };
          ...
          const mergedKeys = state.executionContext.mergeStepResult(step, merged);
          this.reporter.factsMerged(mergedKeys);
          this.completeStep(state, step, 'change-execution-completed');
          continue;
        } catch (error) {
          await this.logger.error('change-execution-error', { stepId: step.id, error: String(error) }, context);
          state.execution.addEvent('change-execution-error', { stepId: step.id, error: String(error) });
          ...
          const recovered = await this.recover(state, `change-execution-error:${String(error)}`);
          if (!recovered) return state.pauseReason ? 'paused' : 'finished';
          continue;
        }
      }
      ...
      if (result.changes.length > 0) {
        const targetPath = result.changes[0]?.path;
        if (!targetPath) {
          const recovered = await this.recover(state, 'model produced changes without a target path');
          if (!recovered) return state.pauseReason ? 'paused' : 'finished';
          continue;
        }
        const direct = await this.changeExecution.executeProposal({
          work: {
            id: `${step.id}:direct-change`,
            goal: step.goal,
            targetPath,
            inputs: step.inputs,
            outputs: step.outputs,
        ...
        if (direct.status === 'failed') {
          const recovered = await this.recover(state, `change-execution-error:${direct.error}`);
          if (!recovered) return state.pauseReason ? 'paused' : 'finished';
          continue;
        }
