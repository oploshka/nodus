 ...
 * component that understands their semantics (Target, Worker, Model, etc.).
 */
export class ConfigurationLoader {
  public static async load(path: string): Promise<AppConfiguration> {
    ...
    if (!parsed.target?.id || !parsed.target.root) {
      throw new Error('Configuration requires target.id and target.root');
    }
    ...
    return {
      target: {
        ...parsed.target,
        id: parsed.target.id,
        root: resolve(base, parsed.target.root),
      },
      model: {
        ...parsed.model,
