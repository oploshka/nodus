...
export class ProjectFiles {
  ...
  public constructor(
    ...
    this.pathResolver = new PathResolver(configuration.root);
    this.configuration = {
      ...configuration,
  ...
  public candidateFiles(question: string, limit = 6): FileFact[] {
    ...
    return scored.filter((item) => item.score > 0).sort((a, b) => b.score - a.score || a.file.path.localeCompare(b.file.path)).slice(0, limit).map((item) => item.file);
  }
