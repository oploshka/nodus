...
import type { ProjectFileFact, ProjectIndex } from '@project/Index/ProjectIndex';

const SOURCE_EXTENSIONS = new Set(['.ts', '.tsx', '.js', '.jsx', '.mjs', '.cjs', '.vue']);

export class ProjectScanner {
  public async scan(configuration: ProjectConfiguration): Promise<ProjectIndex> {
  ...
  private async walk(
    ...
    for (const entry of entries) {
      ...
      if (this.isExcluded(projectPath, configuration.exclude ?? [])) {
        continue;
      }
      ...
      if (!entry.isFile() || !this.isIncluded(projectPath, configuration.include ?? [])) {
        continue;
      }
