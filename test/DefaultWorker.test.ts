...
import type { Planner } from '../src/engine/planner/Planner.js';
...
test('DefaultWorker aggregates planner and available actions', async () => {
  const calls: string[] = [];
  ...
  assert.equal(result.state.history[0].actionId, 'note');
});

test('DefaultWorker enforces action usage limit outside ExecutionPlanner', async () => {
  const action: ExecutionAction = {
    id: 'research',
  ...
  if (result.status === 'failed') assert.match(result.error, /limit exceeded/);
});
